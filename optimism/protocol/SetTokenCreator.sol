...
contract SetTokenCreator {
        ...
        returns (address)
    {
        ...
        // ovm compiler pushes SetCreator's size just over limit to 24.30 kb
        // removing require reasons brings it back down to 23.60 kb
        //
        // Reason: "Must have at least 1 component"
        require(_components.length > 0);
        // Reason: "Component and unit lengths must be the same"
        require(_components.length == _units.length);
        // Reason: "Components must not have a duplicate"
        require(!_components.hasDuplicate());
        // Reason: "Must have at least 1 module"
        require(_modules.length > 0);
        // Reason: ""Manager must not be empty""
        require(_manager != address(0));

        for (uint256 i = 0; i < _components.length; i++) {
            // Reason: "Component must not be null address"
            require(_components[i] != address(0));
            // Reason: "Units must be greater than 0"
            require(_units[i] > 0);
        }

        for (uint256 j = 0; j < _modules.length; j++) {
            // Reason: "Must be enabled module"
            require(controller.isModule(_modules[j]));
        }
        // <<<< OPTIMISM:END_CHANGE
