...
contract VelodromePair is IVelodromePair {
    ...
    constructor() {
        factory = msg.sender;
        (address _token0, address _token1, bool _stable) = VelodromeFactory(msg.sender).getInitializable();
        (token0, token1, stable) = (_token0, _token1, _stable);
        fees = address(new VelodromePairFees(_token0, _token1));
        if (_stable) {
    ...
    function claimFees() external returns (uint claimed0, uint claimed1) {
        ...
        if (claimed0 > 0 || claimed1 > 0) {
            ...
            VelodromePairFees(fees).claimFeesFor(msg.sender, claimed0, claimed1);
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        require(!VelodromeFactory(factory).isPaused());
        ...
        if (data.length > 0) IVelodromePairCallee(to).hook(msg.sender, amount0Out, amount1Out, data); // callback, used for flash loans
        _balance0 = IERC20(_token0).balanceOf(address(this));
        _balance1 = IERC20(_token1).balanceOf(address(this));
        }
        ...
        (address _token0, address _token1) = (token0, token1);
        if (amount0In > 0) _update0(amount0In * VelodromeFactory(factory).getFee(stable) / 10000); // accrue fees for token0 and move them out of pool
        if (amount1In > 0) _update1(amount1In * VelodromeFactory(factory).getFee(stable) / 10000); // accrue fees for token1 and move them out of pool
    ...
    function getAmountOut(uint amountIn, address tokenIn) external view returns (uint) {
        (uint _reserve0, uint _reserve1) = (reserve0, reserve1);
        amountIn -= amountIn * VelodromeFactory(factory).getFee(stable) / 10000; // remove fee from amount received
