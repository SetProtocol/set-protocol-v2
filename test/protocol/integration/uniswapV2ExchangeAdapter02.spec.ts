...
import { UniswapV2ExchangeAdapter02 } from "@utils/contracts";
...
describe("UniswapV2ExchangeAdapter02", () => {
  let owner: Account;
  ...
  let uniswapV2ExchangeAdapter: UniswapV2ExchangeAdapter02;
  ...
  before(async () => {
    ...
    uniswapV2ExchangeAdapter = await deployer.adapters.deployUniswapV2ExchangeAdapter02(uniswapSetup.router.address);
  });
  ...
  describe("constructor", async () => {
    ...
    async function subject(): Promise<any> {
      return await deployer.adapters.deployUniswapV2ExchangeAdapter02(subjectUniswapRouter);
    }

    it("should have the correct router address", async () => {
      const deployedUniswapV2ExchangeAdapter02 = await subject();

      const actualRouterAddress = await deployedUniswapV2ExchangeAdapter02.router();
  ...
  describe("getTradeCalldata", async () => {
    ...
    describe("when passed in custom path and function selection to trade data", async () => {
      beforeEach(async () => {
