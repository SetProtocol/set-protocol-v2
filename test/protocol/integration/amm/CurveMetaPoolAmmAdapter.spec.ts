...
describe("CurveMetaPoolAmmAdapter", () => {
  let owner: Account;
  let user:Account;
  ...
  describe("test", async () => {
    it("should do things", async () => {
      await curveAmmSetup.initializePool(owner.address);
      const signer = provider.getSigner(owner.address);
      console.log("stable",curveAmmSetup.stableSwap.address)
      console.log("deposit", curveAmmSetup.stableSwap.address);
      console.log(curveAmmSetup.dai.address);
      console.log(curveAmmSetup.usdc.address);
      console.log(curveAmmSetup.usdt.address);
      console.log("dai", await (await curveAmmSetup.dai.balanceOf(owner.address)).toString());
      console.log("usdc", await (await curveAmmSetup.usdc.balanceOf(owner.address)).toString());
      console.log("usdt", await (await curveAmmSetup.usdt.balanceOf(owner.address)).toString());
      //console.log("usdn", await (await curveAmmSetup.usdn.balanceOf(owner.address)).toString());
      console.log("coins",await curveAmmSetup.stableSwap.coins(0,{gasLimit:900000}))
      console.log("virtual", await (await curveAmmSetup.stableSwap.get_virtual_price()).toString())
      // console.log(await (await curveAmmSetup.triPool.A()).toString());
      // console.log(await curveAmmSetup.triPool.coins(0, { gasLimit: 90000 }));
      // console.log(await curveAmmSetup.triPool.coins(1, { gasLimit: 90000 }));
      // console.log(await curveAmmSetup.triPool.coins(2, { gasLimit: 90000 }));
      // console.log(await curveAmmSetup.triPool.unkill_me())
      //console.log("virtual",await (await curveAmmSetup.triPool.get_virtual_price({gasLimit:900000})).toString())
      //console.log(await curveAmmSetup.triPool.calc_token_amount([1000000000000000000, 1000000,1000000],true))
      // await curveAmmSetup.triPool.connect(provider.getSigner(user.address))
      //   .add_liquidity(
      //     [
      ...
      //     },
      //   );
      // console.log("triPool", await curveAmmSetup.dai.balanceOf(curveAmmSetup.triPool.address));
      // await curveAmmSetup.threeCrv.connect(signer).mint(owner.address, ether(10));
      // console.log((await curveAmmSetup.threeCrv.totalSupply({ gasLimit: 900000 })).toString());
      // console.log(await (await curveAmmSetup.triPool.get_virtual_price()).toString());
      //console.log(curveAmmSetup.triPool.address);
