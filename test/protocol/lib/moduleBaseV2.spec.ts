...
import { ModuleBaseV2Mock, SetToken, StandardTokenMock, StandardTokenWithFeeMock } from "@utils/contracts";
...
describe("ModuleBaseV2", () => {
  let owner: Account;
  let otherAccount: Account;
  let deployer: DeployHelper;

  let setup: SystemFixture;
  let moduleBase: ModuleBaseV2Mock;
  ...
  describe("#constructor", async () => {
    let subjectModuleBase: ModuleBaseV2Mock;

    async function subject(): Promise<ModuleBaseV2Mock> {
      return deployer.mocks.deployModuleBaseV2Mock(setup.controller.address);
    }
  ...
  context("when there is a SetToken with an enabled module", async () => {
    ...
    beforeEach(async () => {
      moduleBase = await deployer.mocks.deployModuleBaseV2Mock(setup.controller.address);
