...
import { ADDRESS_ZERO, MAX_UINT_256, PRECISE_UNIT, ONE, THREE, ZERO, ONE_DAY_IN_SECONDS } from "@utils/constants";
import {
...
describe("GeneralIndexModuleV2", () => {
  ...
  let indexModule: GeneralIndexModuleV2;
  ...
  before(async () => {
    ...
    indexModule = await deployer.modules.deployGeneralIndexModuleV2(
  ...
  describe("#initialize", async () => {
    ...
    let subjectRebateRecipient: Address;
    let subjectCaller: Account;

    beforeEach(async () => {
      subjectSetToken = index;
      subjectRebateRecipient = owner.address;
      subjectCaller = owner;
    });

    async function subject(): Promise<any> {
      indexModule = indexModule.connect(subjectCaller.wallet);
      return indexModule.initialize(subjectSetToken.address, subjectRebateRecipient);
    }
    ...
    it("should set the manager rebate recipient", async () => {
      await subject();
      const managerRebateRecipient = await indexModule.managerRebateRecipient(subjectSetToken.address);
      expect(managerRebateRecipient).to.eq(subjectRebateRecipient);
    });
  ...
  describe("when module is initialized", async () => {
    ...
    async function initSetToken(
      ...
      await indexModule.initialize(setToken.address, owner.address);
    ...
    describe("#updateFeeRecipient", async () => {
      ...
      async function subject(): Promise<any> {
        return indexModule.connect(subjectCaller.wallet).updateFeeRecipient(
          subjectSetToken.address,
      ...
      it("should have set the new fee recipient address", async () => {
        ...
        expect(managerRebateRecipient).to.eq(subjectNewFeeRecipient);
      });

      it("should emit the correct FeeRecipientUpdated event", async () => {
        await expect(subject()).to.emit(indexModule, "FeeRecipientUpdated").withArgs(
          subjectSetToken.address,
          subjectNewFeeRecipient
        );
      });
      ...
      describe("when fee recipient address is same address", async () => {
        beforeEach(async () => {
          subjectNewFeeRecipient = await indexModule.managerRebateRecipient(subjectSetToken.address);
        });
    ...
    describe("#trade", async () => {
      ...
      describe("with default target units", async () => {
        ...
        describe("when there is a protocol fee charged", async () => {
          ...
          let managerRebatePercentage: BigNumber;

          beforeEach(async () => {
            feePercentage = ether(0.001);
            managerRebatePercentage = ether(0.4);
            ...
            await setup.controller.addFee(
              ...
              feePercentage // Set fee to 10 bps
            );

            await setup.controller.addFee(
              indexModule.address,
              ONE, // Fee type on trade function denoted as 0
              managerRebatePercentage // Set rebate to 40%
            );
          });
          ...
          it("the fees should be sent to the correct recipients", async () => {
            const feeRecipient = await setup.controller.feeRecipient();
            const beforeProtocolWethBalance = await setup.weth.balanceOf(feeRecipient);
            const beforeManagerWethBalance = await setup.weth.balanceOf(owner.address);

            await subject();

            const postProtocolWethBalance = await setup.weth.balanceOf(feeRecipient);
            const postManagerWethBalance = await setup.weth.balanceOf(owner.address);

            const totalFee = feePercentage.mul(expectedOut).div(ether(1));
            const managerRebate = preciseMul(totalFee, managerRebatePercentage);
            const protocolFee = totalFee.sub(managerRebate);

            const expectedProtocolWethBalance = beforeProtocolWethBalance.add(protocolFee);
            const expectedManagerWethBalance = beforeManagerWethBalance.add(managerRebate);

            expect(postProtocolWethBalance).to.eq(expectedProtocolWethBalance);
            expect(postManagerWethBalance).to.eq(expectedManagerWethBalance);
          });

          it("emits the correct TradeExecuted event", async () => {
            const totalFee = expectedOut.mul(feePercentage).div(ether(1));
            const managerRebate = preciseMul(totalFee, managerRebatePercentage);
            const protocolFee = totalFee.sub(managerRebate);
    ...
    describe("#tradeRemainingWETH", async () => {
      ...
      describe("with default target units", () => {
        ...
        describe("when ETH remaining in contract, trade remaining WETH", async () => {
          ...
          describe("when protocol fee is charged", async () => {
            let subjectFeePercentage: BigNumber;
            let subjectManagerRebateSplit: BigNumber;

            beforeEach(async () => {
              subjectFeePercentage = ether(0.05);
              subjectManagerRebateSplit = ether(0.3);
              setup.controller = setup.controller.connect(owner.wallet);
              await setup.controller.addFee(
                indexModule.address,
                ZERO, // Fee type on trade function denoted as 0
                subjectFeePercentage // Set fee to 50 bps
              );

              await setup.controller.addFee(
                indexModule.address,
                ONE, // Fee type on trade function denoted as 1
                subjectManagerRebateSplit // Set split to 30%
              );
            });
            ...
            it("the fees should be sent to the correct recipients", async () => {
              const feeRecipient = await setup.controller.feeRecipient();
              const beforeProtocolWbtcBalance = await setup.wbtc.balanceOf(feeRecipient);
              const beforeManagerWbtcBalance = await setup.wbtc.balanceOf(owner.address);

              await subject();

              const postProtocolWbtcBalance = await setup.wbtc.balanceOf(feeRecipient);
              const postManagerWbtcBalance = await setup.wbtc.balanceOf(owner.address);

              const totalFee = subjectFeePercentage.mul(expectedWbtcOut).div(ether(1));
              const managerRebate = preciseMul(totalFee, subjectManagerRebateSplit);
              const protocolFee = totalFee.sub(managerRebate);

              const expectedProtocolWbtcBalance = beforeProtocolWbtcBalance.add(protocolFee);
              const expectedManagerWbtcBalance = beforeManagerWbtcBalance.add(managerRebate);

              expect(postProtocolWbtcBalance).to.eq(expectedProtocolWbtcBalance);
              expect(postManagerWbtcBalance).to.eq(expectedManagerWbtcBalance);
            });

            it("emits the correct TradeExecuted event", async () => {
              const totalFee = expectedWbtcOut.mul(subjectFeePercentage).div(ether(1));
              const managerRebate = preciseMul(totalFee, subjectManagerRebateSplit);
              const protocolFee = totalFee.sub(managerRebate);
              await expect(subject()).to.be.emit(indexModule, "TradeExecuted").withArgs(
                subjectSetToken.address,
    ...
    describe("#removeModule", async () => {
      ...
      async function restoreModule() {
        ...
        if (!isModuleEnabled) {
          ...
          await indexModule.connect(subjectCaller.wallet).initialize(subjectSetToken.address, owner.address);
        }
      }
      ...
      describe("removal", async () => {
        ...
        it("should delete the manager recipient state", async () => {
          await subject();
          const managerRebateRecipient = await indexModule.managerRebateRecipient(subjectSetToken.address);
          expect(managerRebateRecipient).to.eq(ADDRESS_ZERO);
        });
      });
