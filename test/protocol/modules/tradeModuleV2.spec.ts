...
import { ADDRESS_ZERO, EMPTY_BYTES, MAX_UINT_256, ZERO, ONE } from "@utils/constants";
...
describe("TradeModuleV2", () => {
  ...
  let tradeModuleV2: TradeModuleV2;
  ...
  cacheBeforeEach(async () => {
    ...
    tradeModuleV2 = await deployer.modules.deployTradeModuleV2(setup.controller.address);
    await setup.controller.addModule(tradeModuleV2.address);

    await setup.integrationRegistry.batchAddIntegration(
      [
        tradeModuleV2.address,
        tradeModuleV2.address,
        tradeModuleV2.address,
        tradeModuleV2.address,
        tradeModuleV2.address,
  ...
  describe("#constructor", async () => {
    let subjectTradeModuleV2: TradeModuleV2;

    async function subject(): Promise<TradeModuleV2> {
      return deployer.modules.deployTradeModuleV2(setup.controller.address);
    }

    it("should have the correct controller", async () => {
      subjectTradeModuleV2 = await subject();
      const expectedController = await subjectTradeModuleV2.controller();
      expect(expectedController).to.eq(setup.controller.address);
    });
  });

  context("when there is a deployed SetToken with enabled TradeModuleV2", async () => {
    ...
    cacheBeforeEach(async () => {
      ...
      setToken = await setup.createSetToken(
        [sourceToken.address],
        [wbtcUnits],
        [setup.issuanceModule.address, tradeModuleV2.address],
        manager.address
      );
    });
    ...
    describe("#initialize", async () => {
      ...
      beforeEach(async () => {
        ...
        subjectRebateRecipient = manager.address;
        subjectCaller = manager;
      });

      async function subject(): Promise<any> {
        tradeModuleV2 = tradeModuleV2.connect(subjectCaller.wallet);
        return tradeModuleV2.initialize(subjectSetToken, subjectRebateRecipient);
      }

      it("should set the manager rebate recipient", async () => {
        await subject();
        const managerRebateRecipient = await tradeModuleV2.managerRebateRecipient(setToken.address);
        expect(managerRebateRecipient).to.eq(manager.address);
      });

      it("should enable the Module on the SetToken", async () => {
        await subject();
        const isModuleEnabled = await setToken.isInitializedModule(tradeModuleV2.address);
    ...
    describe("#trade", async () => {
      ...
      context("when trading a Default component on Kyber", async () => {
        ...
        const initializeContracts = async () => {
          ...
          if (isInitialized) {
            tradeModuleV2 = tradeModuleV2.connect(manager.wallet);
            await tradeModuleV2.initialize(setToken.address, manager.address);
          }
        ...
        describe("when the module is initialized", () => {
          ...
          describe("when there is a protocol fee charged", async () => {
            ...
            it("should transfer the correct components minus fee to the SetToken", async () => {
              ...
              await subject();
              const totalDestinationQuantity = issueQuantity.mul(destinationTokenQuantity).div(ether(1));
              const totalFee = feePercentage.mul(totalDestinationQuantity).div(ether(1));
            ...
            it("should transfer the fees to the correct recipients", async () => {
              const protocolFeeRecipient = await setup.controller.feeRecipient();
              const preProtocolFeeRecipientBalance = await destinationToken.balanceOf(protocolFeeRecipient);
              const preManagerRebateRecipientBalance = await destinationToken.balanceOf(manager.address);
              ...
              const postProtocolFeeRecipientBalance = await destinationToken.balanceOf(protocolFeeRecipient);
              const postManagerRebateRecipientBalance = await destinationToken.balanceOf(manager.address);

              expect(postProtocolFeeRecipientBalance.sub(preProtocolFeeRecipientBalance)).to.eq(protocolFee);
              expect(postManagerRebateRecipientBalance.sub(preManagerRebateRecipientBalance)).to.eq(managerRebate);
            });
            ...
            it("should emit the correct ComponentExchanged event", async () => {
              ...
              await expect(subject()).to.emit(tradeModuleV2, "ComponentExchanged").withArgs(
                setToken.address,
                subjectSourceToken,
                subjectDestinationToken,
                kyberExchangeAdapter.address,
                totalSourceQuantity,
                totalDestinationQuantity.sub(totalFee),
                protocolFee,
                managerRebate
              );
            });
      ...
      context("when trading a Default component with a transfer fee on Uniswap", async () => {
        cacheBeforeEach(async () => {
          ...
          await tradeModuleV2.connect(manager.wallet).trade(
            setToken.address,
            uniswapTransferFeeAdapterName,
            setup.wbtc.address,
            wbtcSendQuantity,
    ...
    describe("#updateFeeRecipient", async () => {
      ...
      async function subject(): Promise<any> {
        return tradeModuleV2.connect(subjectCaller.wallet).updateFeeRecipient(
          subjectSetToken,
      ...
      it("should have set the new fee recipient address", async () => {
        ...
        const managerRebateRecipient = await tradeModuleV2.managerRebateRecipient(subjectSetToken);

        expect(managerRebateRecipient).to.eq(subjectNewFeeRecipient);
      });

      it("should emit the correct FeeRecipientUpdated event", async () => {
        await expect(subject()).to.emit(tradeModuleV2, "FeeRecipientUpdated").withArgs(
          subjectSetToken,
          subjectNewFeeRecipient
        );
      });
      ...
      describe("when fee recipient address is same address", async () => {
        beforeEach(async () => {
          subjectNewFeeRecipient = await tradeModuleV2.managerRebateRecipient(subjectSetToken);
        });
    ...
    describe("#removeModule", async () => {
      ...
      beforeEach(async () => {
        ...
        subjectModule = tradeModuleV2.address;
      });
