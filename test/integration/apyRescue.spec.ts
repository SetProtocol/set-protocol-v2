...
describe.only("APYRescue [ @forked-mainnet ]", () => {
  ...
  before(async () => {
    ...
    ethusdToken = await deployer.core.getSetToken("0x23687D9d40F9Ecc86E7666DDdB820e700F954526");
    weth = await deployer.external.getWETH("0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2");

    apyRescue = await deployer.product.deployAPYRescue(ethusdToken.address, weth.address, basicIssuanceModule.address);
  ...
  describe("#recoverAssets", async () => {
    ...
    it("should redeem all ETHUSD tokens in the APYRescue contract", async () => {
      const preBalance = await ethusdToken.balanceOf(apyRescue.address);
      expect(preBalance).to.eq(ether(50));

      await subject();

      const postBalance = await ethusdToken.balanceOf(apyRescue.address);
  ...
  describe("#withdrawRecoveredFunds", async () => {
    ...
    it("should withdraw all WETH tokens in the APYRescue contract", async () => {
      const preRescueBalance = await weth.balanceOf(apyRescue.address);
      const preOwnerBalance = await weth.balanceOf(setOwner.address);

      await subject();

      const postRescueBalance = await weth.balanceOf(apyRescue.address);
      const postOwnerBalance = await weth.balanceOf(setOwner.address);

      expect(postRescueBalance).to.eq(0);
      expect(postOwnerBalance).to.eq(preOwnerBalance.add(preRescueBalance));
    });
  });
});
